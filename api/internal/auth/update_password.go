...
func (s *Service) UpdatePasswordHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := s.pool.Exec(r.Context(), `UPDATE app_users SET password_hash = $1 WHERE id = $2`, string(hash), u.AppUserID); err != nil {
		httpx.ProblemJSON(w, http.StatusInternalServerError, "Server Error", err.Error(), "internal")
		return
	}
	s.RevokeAllSessions(r.Context(), u.AppUserID, "password_changed")
	s.audit(r.Context(), &u.AppUserID, "password_changed", map[string]any{})
